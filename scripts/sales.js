...
export async function loadSales(filter = {}) {
  ...
  currentSales = allSales.filter((sale) => {
    ...
    if (filter.category && filter.category.length > 0) {
      const hasCategory = sale.products.some(
        (p) =>
          p.category &&
...
function renderSalesTable(sales) {
  ...
  document.getElementById("salesPaginationInfo").textContent = 
    `Mostrando ${startIndex + 1}-${endIndex} de ${sales.length} ventas`;

  // Actualizar controles de paginación
  updateSalesPaginationControls(sales.length);
}
  ...
  try {
    ...
    const statsData = [
      { Métrica: "Total Ventas", Valor: salesData.length },
      {
        ...
        Valor: salesData.reduce((sum, sale) => sum + sale.products.length, 0),
      },
      {
        ...
        <div class="space-y-2">
          ...
          <select id="filterStatus" class="w-full px-4 py-2 border-2 border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition-colors">
            ...
            <option value="Completada" ${
              currentFilters.status === "Completada" ? "selected" : ""
            }>Completada</option>
            <option value="Pendiente" ${
              currentFilters.status === "Pendiente" ? "selected" : ""
            }>Pendiente</option>
            <option value="Cancelada" ${
              currentFilters.status === "Cancelada" ? "selected" : ""
            }>Cancelada</option>
          </select>
        </div>
        ...
        <div class="space-y-2">
          ...
          <select id="filterCategory" class="w-full px-4 py-2 border-2 border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition-colors">
            ...
            <option value="Electrónica" ${
              currentFilters.category === "Electrónica" ? "selected" : ""
            }>Electrónica</option>
            <option value="Ropa" ${
              currentFilters.category === "Ropa" ? "selected" : ""
            }>Ropa</option>
            <option value="Hogar" ${
              currentFilters.category === "Hogar" ? "selected" : ""
            }>Hogar</option>
            <option value="Alimentos" ${
              currentFilters.category === "Alimentos" ? "selected" : ""
            }>Alimentos</option>
          </select>
        </div>
      </div>
...
// Función para actualizar los controles de paginación de ventas
function updateSalesPaginationControls(totalItems) {
  totalSalesPages = Math.ceil(totalItems / itemsPerPage);
  ...
  const startPage = Math.max(1, currentSalesPage - 2);
  const endPage = Math.min(totalSalesPages, currentSalesPage + 2);

  for (let i = startPage; i <= endPage; i++) {
    const pageBtn = document.createElement("button");
    pageBtn.className = `px-3 py-1 border rounded ${i === currentSalesPage ? "bg-blue-500 text-white" : "bg-white"}`;
...
// Event listeners para los botones de paginación de ventas
